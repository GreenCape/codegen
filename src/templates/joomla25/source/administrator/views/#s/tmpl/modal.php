...
{% if entity.role != 'map' and entity.special.key %}
<?php
/**
...
$action = 'index.php?option=com_{{ project.name | file }}&view={{ entity.name | plural | file }}&layout=modal&tmpl=component&{% if entity.role != 'lookup' %}function=' . $function . '&{% endif %}' . JSession::getFormToken() . '=1';
?>
...
{% if entity.special.title %}'COM_{{ project.name | constant }}_{{ entity.name | constant }}_FIELD_{{ entity.special.title.name | constant }}_LABEL', '{{ entity.name }}.{{ entity.special.title.name | variable }}'{% else %}'COM_{{ project.name | constant }}_{{ entity.name | constant }}_ITEM', 'name'{% endif %}, $listDirn, $listOrder); ?>
				</th>
{% endif %}
...
{% if entity.special.title %}
					<?php
					$onClick = sprintf("if (window.parent) window.parent.%s('%d', '%s', {% if entity.special.category %}'%d', {% endif %}null);",
						$this->escape($function),
						$item->id,
						$this->escape($item->title){% if entity.special.category %},
...
{% if entity.dynName %}
					...
					<?php
					$title   = implode(' ', array({% for property in entity.dynName %}$item->{{ property.name | variable }}, {% endfor %}));
					$onClick = sprintf("if (window.parent) window.parent.%s('%d', '%s', {% if entity.special.category %}'%d', {% endif %}null);",
						$this->escape($function),
						$item->id,
						$this->escape($title){% if entity.special.category %},
						$this->escape($item->catid)
{% endif %}
					);
					?>
