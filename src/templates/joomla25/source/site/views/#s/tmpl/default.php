...
<?php
/**
 * {{ project.title }} {{ entity.name | title }} HTML List Template
 *
 * PHP version 5.3
 *
{# align("\t", '  ') #}
...
{% if entity.special.publish_up %}
							...
							<?php echo JHtml::_('grid.sort', 'COM_{{ project.name | constant }}_{{ entity.name | constant }}_FIELD_{{ entity.special.published.name | constant }}_LABEL', '{{ entity.name }}.{{ entity.special.publish_up.name }}', $listDirn, $listOrder); ?>
							<?php endif; ?>
{% endif %}
...
{% if entity.special.created_by %}
						...
						<th class="list-author" id="tableOrdering3">
							<?php echo JHtml::_('grid.sort', 'COM_{{ project.name | constant }}_{{ entity.name | constant }}_FIELD_{{ entity.special.created_by.name | constant }}_LABEL', 'author', $listDirn, $listOrder); ?>
						</th>
						<?php endif; ?>
{% endif %}
...
{% endif %}
						...
						<td class="list-title">
							<a href="<?php echo JRoute::_({{ project.name | class }}HelperRoute::get{{ entity.name | class }}Route($item->{{ entity.special.key.name }}{% if entity.special.category %}, $item->{{ entity.special.category.name }}{% endif %})); ?>">
...
{% if entity.special.access %}
						...
						<td>
							...
							$returnURL  = JRoute::_({{ project.name | class }}HelperRoute::get{{ entity.name | class }}Route($item->{{ entity.special.key.name }}));
...
{% endif %}
			...
			<?php if (JFactory::getUser()->authorize('core.create')) : ?>
			<span class="actions">
				<span class="edit-icon">
					<?php echo JHtml::_('icon.create', null, '{{ entity.name | singular | file }}', $this->params); ?>
				</span>
			</span>
			<?php  endif; ?>
