...
<?php
/**
 * {{ project.title }} {{ entity.name | title }} Frontend List Model Tests
 *
 * PHP version 5.3
 *
{# align("\t", '  ') #}
...
require_once SOURCE_DIR . '/site/models/{{ entity.name | plural | file }}.php';

/**
 * Tests for {{ entity.name | class }} List Model
 *
{# align("\t", '  ') #}
...
{# endalign #}
 */
class {{ project.name | class }}Model{{ entity.name | plural | class }}Test extends PHPUnit_Framework_TestCase
{
	/**
	 * The object under test
	 *
	 * @var  {{ project.name | class }}Model{{ entity.name | plural | class }}
	 */
	protected $object;
	...
	{
		$this->object = new {{ project.name | class }}Model{{ entity.name | plural | class }};
	}
